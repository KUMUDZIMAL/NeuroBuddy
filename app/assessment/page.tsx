...
export default function MentalHealthQuestionnaire() {
  ...
  const analyzeResponses = async (finalResponses: {question: string, answer: string}[]) => {
    ...
    try {
      ...
      // Parse and save diagnosis to localStorage
      try {
        const conditions: { [key: string]: number } = {};
        const lines = data.insights.split('\n');
        
        for (const line of lines) {
          // Match pattern: "Condition Name (Confidence: XX%)"
          const match = line.match(/(.+?)\s*\(Confidence:\s*(\d+)%\)/);
          if (match) {
        ...
        if (Object.keys(conditions).length > 0) {
          ...
          localStorage.setItem('assessment_diagnosis', JSON.stringify(diagnosisData));
          console.log('Saved diagnosis to localStorage:', diagnosisData);
        }
      } catch (parseError) {
        console.error('Error parsing insights for localStorage:', parseError);
      }
    } catch (error) {
      console.error('Error analyzing responses:', error);
    } finally {
